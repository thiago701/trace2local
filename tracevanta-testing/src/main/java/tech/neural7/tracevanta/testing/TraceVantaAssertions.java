 ...
 * "o TraceVanta viu X" DEVE ter o gêmeo negativo "o TraceVanta declarou que não
 * viu Y" — por isso cada asserção positiva tem uma negativa correspondente.
 */
public final class TraceVantaAssertions {

    private TraceVantaAssertions() {}

    public static Tracer tracer() {
        return io.opentelemetry.api.GlobalOpenTelemetry.get()
                .getTracer("tech.neural7.tracevanta:testing");
    }
    ...
    public static Span startTestSpan(String name) {
        return tracer().spanBuilder(name)
                .setAttribute(tech.neural7.tracevanta.otel.TraceVantaAttributes.TRIGGER,
                        tech.neural7.tracevanta.otel.TraceVantaAttributes.TRIGGER_TEST)
                .startSpan();
    }
    ...
    public static Execution awaitLatest(Duration timeout) {
        ...
        while (System.nanoTime() < deadline) {
            ...
            boolean nothingInFlight = TraceVantaRuntime.pipeline().store().liveExecutions() == 0;
            if (!waitedEnough || !nothingInFlight) {
                continue;
            }
            var recent = TraceVantaRuntime.pipeline().store().recent(50);
            if (!recent.isEmpty()) {
                var execution = TraceVantaRuntime.pipeline().store().get(recent.get(0).executionId());
    ...
    private static Execution awaitById(String executionId, Duration timeout) {
        long deadline = System.nanoTime() + timeout.toNanos();
        while (System.nanoTime() < deadline) {
            var execution = TraceVantaRuntime.pipeline().store().get(executionId);
            if (execution.isPresent()) {
                return execution.get();
            }
            sleep(20);
    ...
    public static final class ExecutionAssert extends AbstractAssert<ExecutionAssert, Execution> {
        ...
        public ExecutionAssert hasStatus(tech.neural7.tracevanta.model.ExecutionStatus status) {
