...
class TraceVantaLambdaRuntimeTest {

    @Test
    void flushesSynchronouslyAtTheEndOfInvocation() throws Exception {
        List<SpanData> exported = new ArrayList<>();
        TraceVantaLambdaRuntime runtime = TraceVantaLambdaRuntime.create(
                TraceVantaConfig.defaults(), new InMemoryExporter(exported));
        ...
        assertThat(span.getAttributes().get(io.opentelemetry.api.common.AttributeKey.stringKey("tv.trigger")))
    ...
    void handlerBaseClassWrapsDelegation() throws Exception {
        ...
        TraceVantaLambdaRuntime runtime = TraceVantaLambdaRuntime.create(
                TraceVantaConfig.defaults(), new InMemoryExporter(exported));
        var handler = new TraceVantaLambdaHandler<String, String>(runtime) {
            @Override
    ...
    void configFromEnvReadsStationToken() {
        ...
        System.setProperty("tracevanta.station.endpoint", "http://127.0.0.1:19876");
        System.setProperty("tracevanta.station.token", "prop-token");
        try {
            TraceVantaConfig cfg = TraceVantaLambda.configFromEnv();
            assertThat(cfg.stationEndpoint()).isEqualTo("http://127.0.0.1:19876");
            assertThat(cfg.stationToken()).isEqualTo("prop-token");
        } finally {
            System.clearProperty("tracevanta.station.endpoint");
            System.clearProperty("tracevanta.station.token");
        }
    }
