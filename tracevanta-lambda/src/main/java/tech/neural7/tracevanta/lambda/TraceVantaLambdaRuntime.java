...
import tech.neural7.tracevanta.otel.TraceVantaAttributes;
 ...
 * ({@code tracevanta.flush-timeout-ms}, padrão 200 ms) com descarte silencioso
 * ao estourar (SPEC §4.2).
 */
public final class TraceVantaLambdaRuntime {
    ...
    private TraceVantaLambdaRuntime(TraceVantaConfig cfg, SpanExporter exporter) {
        this.cfg = cfg;
        ...
        this.sdk = OpenTelemetrySdk.builder()
                ...
                .build();
        this.tracer = sdk.getTracer("tech.neural7.tracevanta:lambda");
        this.mutationPublisher = cfg.stationEndpoint() != null && !cfg.stationEndpoint().isBlank()
                ...
                : null;
        // os instrumentos (AWS SDK, JDBC, spans manuais) leem TraceVantaOtel.get():
    ...
    public static TraceVantaLambdaRuntime forStation(TraceVantaConfig cfg) {
        if (cfg.stationEndpoint() == null || cfg.stationEndpoint().isBlank()) {
            throw new IllegalStateException(
                    "tracevanta.station.endpoint é obrigatório no modo Lambda (ADR-002) — "
                    + "defina a env TRACEVANTA_STATION_ENDPOINT ou use o construtor com config explícita.");
        }
        ...
        return new TraceVantaLambdaRuntime(cfg, builder.build());
    }

    /** Cria com exportador customizado (testes, outros backends). */
    public static TraceVantaLambdaRuntime create(TraceVantaConfig cfg, SpanExporter exporter) {
        return new TraceVantaLambdaRuntime(cfg, exporter);
    }

    private static String stationTracesUrl(TraceVantaConfig cfg) {
    ...
    public <T> T around(Context lambdaContext, io.opentelemetry.api.trace.SpanContext remoteParent,
        ...
        io.opentelemetry.api.trace.SpanBuilder builder = tracer.spanBuilder(
                ...
                .setAttribute(TraceVantaAttributes.TRIGGER, TraceVantaAttributes.TRIGGER_LAMBDA)
                .setAttribute(TraceVantaAttributes.EXECUTION_ID, requestId(lambdaContext))
                .setAttribute(tech.neural7.tracevanta.otel.OtelAttributeNames.FAAS_NAME,
                        lambdaContext != null ? lambdaContext.getFunctionName() : "?")
                .setAttribute(tech.neural7.tracevanta.otel.OtelAttributeNames.FAAS_INVOCATION_ID,
