/* Trace2Local UI — vanilla JS, zero dependência externa (ADR-005).
...
window.__t2lState = state; // auxílio de debug/evidência (removível)
...
function storyToMarkdown(story) {
  const lines = [];
  lines.push("# " + (story.title || "Jornada Trace2Local"));
  lines.push("");
...
function layout(roots) {
  ...
  for (let i = roots.length - 1; i >= 0; i--) {
    ...
    if (kids.length === 0) {
      ...
      const xs = kids.map((k) => k.__t2lX).filter((v) => v !== undefined);
      x = xs.length > 0 ? (xs[0] + xs[xs.length - 1]) / 2 : leafCount.n * (W + GX);
    }
    node.__t2lX = x;
    all.push({ node, x, y: (node.__t2lDepth || 0) * (H + GY), depth: node.__t2lDepth || 0 });
  }
  // profundidades: percorre a árvore do topo
  const assignDepth = (node, depth) => {
    node.__t2lDepth = depth;
  ...
  for (const item of all) {
    out.push({ node: item.node, x: item.x, y: item.node.__t2lDepth * (H + GY), depth: item.node.__t2lDepth });
  }
  for (const n of roots) { delete n.__t2lX; delete n.__t2lDepth; }
