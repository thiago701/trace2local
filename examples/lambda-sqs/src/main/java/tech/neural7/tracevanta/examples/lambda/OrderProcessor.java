 ...
 * <ol>
 *   <li>grava o pedido no DynamoDB via {@link TraceVantaAws#instrument} — o
 ...
 *   <li>o {@link TraceVantaLambdaHandler} abre o span raiz (SERVER com
 *       {@code faas.name} → nó LAMBDA), correlaciona as mutações e faz o flush
 *       síncrono OTLP + {@code /tvingest/v1/mutations} para o Station (ADR-002).</li>
 * </ol>
 *
 ...
 */
public final class OrderProcessor extends TraceVantaLambdaHandler<Map<String, String>, String> {
    ...
    /** Modo deploy: configuração via ambiente ({@code TRACEVANTA_STATION_ENDPOINT}). */
    public OrderProcessor() {
        this(TraceVantaLambda.configFromEnv());
    }

    /** Modo teste/embarcado: config explícita (Station do IT em processo). */
    public OrderProcessor(TraceVantaConfig cfg) {
        super(cfg);
    ...
    private void sendToQueue(String orderId, String customerId) {
        io.opentelemetry.api.trace.Tracer tracer =
                tech.neural7.tracevanta.otel.TraceVantaOtel.get()
                        .getTracer("tech.neural7.tracevanta.examples:lambda-sqs");
        io.opentelemetry.api.trace.Span producer = tracer.spanBuilder("orders-queue send")
                ...
                .setAttribute(tech.neural7.tracevanta.otel.OtelAttributeNames.MESSAGING_SYSTEM, "aws_sqs")
                .setAttribute(tech.neural7.tracevanta.otel.OtelAttributeNames.MESSAGING_DESTINATION, "orders-queue")
                .setAttribute(tech.neural7.tracevanta.otel.OtelAttributeNames.MESSAGING_OPERATION, "publish")
