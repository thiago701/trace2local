...
> Cenário: **AWS Lambda (runtime java21) + DynamoDB + SQS no LocalStack 4.2, configurado com a lib TraceVanta** (modo Companion, ADR-002).
...
| 11 | **Token de ingest (ADR-007/§8.1)**: Station com `TRACEVANTA_STATION_TOKEN` rejeita OTLP sem Bearer (401), aceita com Bearer — Lambda emulada e DemoRun enviam automaticamente; delta EXACT chega pelo canal de mutação protegido | ✅ 401 sem token · execução COMPLETED com `CREATE/EXACT` via ingest autenticado |
...
aws lambda invoke (LocalStack, runtime java21)
        │
        ▼
OrderProcessor (TraceVantaLambdaHandler — span raiz SERVER com faas.name → LAMBDA)
        │  TraceVantaAws.instrument(DynamoDbClient)  → PutItem (delta EXACT)
        │  span PRODUCER manual + AWSTraceHeader     → SendMessage SQS
        ▼
flush síncrono (ADR-002/§4.2): OTLP → /v1/traces · mutações → /tvingest/v1/mutations
        │
        ▼
...
2. **Trigger/executionId não chegavam pelo OTLP** — o ingest do Station não parseava `tv.trigger`/`tv.execution.id`.
3. **Runtime Lambda não registrava o SDK** no `TraceVantaOtel` — o interceptor lazy do AWS SDK resolvia o global noop.
...
8. **Identidade de execução instável** — span tardio com outro `tv.execution.id` renomeava a execução → o primeiro id explícito (span raiz) vence.
