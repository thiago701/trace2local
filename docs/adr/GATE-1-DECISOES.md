...
| **D-3** | `ReturnValues` elevado por padrão? | **Ligado por padrão em perfil de desenvolvimento**, com aviso no log de boot; desligável via `trace2local.aws.dynamodb.capture-before=false`; desligado fora de dev | ADR-003 sai de "com ressalva" → Aceita |
| **D-4** | groupId | **`tech.neural7.trace2local`** (verificação de domínio `neural7.tech` via TXT antes do primeiro deploy; fallback `io.github.*`) | ADR-010 sai de Proposta → Aceita |
| **D-5** | Grafia oficial | **Trace2Local**; arquivos `traceventa-*` renomeados para `trace2local-*` neste commit | Pendência de §11.2 resolvida |
