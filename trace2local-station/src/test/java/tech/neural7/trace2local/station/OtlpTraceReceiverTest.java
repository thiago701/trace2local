...
class OtlpTraceReceiverTest {
        ...
        try (Trace2LocalPipeline pipeline = Trace2LocalPipeline.start(cfg)) {
            ...
            try {
                ...
                Span root = span(rootId, "", "POST /orders", Span.SpanKind.SPAN_KIND_SERVER, 0, 30,
                        kv(tech.neural7.trace2local.otel.OtelAttributeNames.HTTP_METHOD, "POST"), kv(tech.neural7.trace2local.otel.OtelAttributeNames.HTTP_ROUTE, "/orders"));
                Span sns = span(snsId, rootId, "Sns.Publish", Span.SpanKind.SPAN_KIND_PRODUCER, 5, 15,
                        kv(tech.neural7.trace2local.otel.OtelAttributeNames.RPC_SYSTEM, "aws-api"), kv(tech.neural7.trace2local.otel.OtelAttributeNames.RPC_SERVICE, "Sns"), kv(tech.neural7.trace2local.otel.OtelAttributeNames.RPC_METHOD, "Publish"),
                        kv(tech.neural7.trace2local.otel.OtelAttributeNames.AWS_SNS_TOPIC, "arn:aws:sns:us-east-1:000000000000:order-events"));
        ...
        try (Trace2LocalPipeline pipeline = Trace2LocalPipeline.start(cfg)) {
            ...
            try {
                ...
                Span failing = Span.newBuilder()
                        ...
                        .addAttributes(kv(tech.neural7.trace2local.otel.OtelAttributeNames.RPC_SYSTEM, "aws-api"))
                        .addAttributes(kv(tech.neural7.trace2local.otel.OtelAttributeNames.RPC_SERVICE, "DynamoDb"))
                ...
                assertThat(execution.roots().get(0).status())
                        .isEqualTo(tech.neural7.trace2local.model.NodeStatus.ERROR);
    ...
    void rejectsMalformedProtobuf() throws Exception {
        try (Trace2LocalPipeline pipeline = Trace2LocalPipeline.start(Trace2LocalConfig.defaults())) {
            HttpServer http = HttpServer.create(new InetSocketAddress("127.0.0.1", 0), 4);
            http.createContext("/v1/traces", new OtlpTraceReceiver(pipeline, Trace2LocalConfig.defaults()));
            http.setExecutor(java.util.concurrent.Executors.newVirtualThreadPerTaskExecutor());
            http.start();
            try {
                HttpResponse<String> response = HttpClient.newHttpClient().send(HttpRequest.newBuilder()
    ...
    static Execution awaitExecution(Trace2LocalPipeline pipeline, String traceId, Duration timeout) {
