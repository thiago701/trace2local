...
import tech.neural7.trace2local.otel.DefaultSemanticMapper;
import tech.neural7.trace2local.otel.NodeLabel;
import tech.neural7.trace2local.otel.SemanticMapper;
 ...
 * instrumentado com OTel aponta para o Station sem código do Trace2Local. Um
...
public final class OtlpTraceReceiver implements HttpHandler {
    ...
    public OtlpTraceReceiver(Trace2LocalPipeline pipeline, Trace2LocalConfig cfg) {
    ...
    private void ingest(io.opentelemetry.proto.trace.v1.Span span) {
        ...
        span.getAttributesList().forEach(kv -> {
            ...
            if (cfg.redactionMode() != tech.neural7.trace2local.config.RedactionMode.OFF
                    && tech.neural7.trace2local.internal.Redactor.isSensitiveKey(key)) {
                value = tech.neural7.trace2local.internal.Redactor.REDACTED;
            } else {
                value = tech.neural7.trace2local.internal.Redactor.redactString(value, cfg.redactionMode());
            }
        ...
        tech.neural7.trace2local.model.ErrorInfo errorInfo = null;
        if (error) {
            ...
            errorInfo = new tech.neural7.trace2local.model.ErrorInfo(
        ...
        // t2l.execution.id e t2l.trigger como atributos do span raiz (§4.9/§4.12)
        String executionId = attributes.get(tech.neural7.trace2local.otel.Trace2LocalAttributes.EXECUTION_ID);
