...
class Trace2LocalThreadFactoryTest {
    ...
    void propagatesOtelContextToVirtualThreads() throws InterruptedException {
        ...
        try {
            ...
            try (Scope ignored = Context.current().with(parent).makeCurrent()) {
                Thread thread = Trace2LocalThreadFactory.virtual("tv-test").newThread(() -> {
    ...
    void stillWorksWithoutActiveContext() throws InterruptedException {
        CountDownLatch latch = new CountDownLatch(1);
        Thread thread = Trace2LocalThreadFactory.virtual("tv-null").newThread(latch::countDown);
        thread.start();
        assertThat(latch.await(5, TimeUnit.SECONDS)).isTrue();
    }
    ...
    void delegatesNamePrefix() throws InterruptedException {
        ...
        Thread thread = Trace2LocalThreadFactory.virtual("tv-name").newThread(() -> {
