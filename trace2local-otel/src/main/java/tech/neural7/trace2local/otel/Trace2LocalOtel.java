...
/**
 * Helper de boot do SDK OTel com o processor do Trace2Local ACrescentado ao
 * pipeline (ADR-001) — se o dev já tem exporters configurados, eles continuam;
 * o Trace2Local apenas se soma.
 *
 ...
 * de qualquer bean; portanto os instrumentos do Trace2Local leem de
 ...
 * {@link Trace2LocalOtelConfigurer} (ServiceLoader).
 */
public final class Trace2LocalOtel {

    private static volatile OpenTelemetry own;

    private Trace2LocalOtel() {}

    /** Registra o SDK do Trace2Local no boot do starter. */
    public static void register(OpenTelemetry sdk) {
        own = sdk;
    }

    /** O SDK do Trace2Local quando ele é o dono do pipeline; senão, o global do dev. */
    ...
    public static SpanProcessor processor(Consumer<Trace2LocalEvent> sink, Trace2LocalConfig cfg) {
        return new Trace2LocalSpanProcessor(sink, cfg);
    }
}
