...
class TraceVantaJdbcTest {
    ...
    void createsSpanWithStableDbAttributes() throws Exception {
        TraceVantaConfig cfg = TraceVantaConfig.builder()
                ...
                .build();
        DataSource wrapped = TraceVantaJdbc.wrap(dataSource("UPDATE orders SET total = 251 WHERE pk = 'ORDER#1'", 1), cfg);
        ...
        assertThat(span.getAttributes().get(io.opentelemetry.api.common.AttributeKey.stringKey(tech.neural7.tracevanta.otel.OtelAttributeNames.DB_SYSTEM)))
                .isEqualTo("postgresql");
        assertThat(span.getAttributes().get(io.opentelemetry.api.common.AttributeKey.stringKey(tech.neural7.tracevanta.otel.OtelAttributeNames.DB_OPERATION)))
                .isEqualTo("UPDATE");
        assertThat(span.getAttributes().get(io.opentelemetry.api.common.AttributeKey.stringKey(tech.neural7.tracevanta.otel.OtelAttributeNames.DB_COLLECTION)))
                .isEqualTo("orders");
    }
    ...
    void redactsSqlAtTheSource() throws Exception {
        ...
        DataSource wrapped = TraceVantaJdbc.wrap(dataSource("UPDATE users SET password = 'hunter2'", 1), cfg);
        ...
        String query = span.getAttributes().get(io.opentelemetry.api.common.AttributeKey.stringKey(tech.neural7.tracevanta.otel.OtelAttributeNames.DB_QUERY_TEXT));
    ...
    void publishesInferredMutationWhenConfigured() throws Exception {
        TraceVantaConfig cfg = TraceVantaConfig.builder()
                .jdbcMutationCapture(JdbcMutationCapture.INFERRED)
                .build();
        DataSource wrapped = TraceVantaJdbc.wrap(dataSource("DELETE FROM orders WHERE pk = 'ORDER#9'", 1), cfg);

        wrapped.getConnection().prepareStatement("DELETE FROM orders WHERE pk = 'ORDER#9'").executeUpdate();

        assertThat(mutations).hasSize(1);
        assertThat(mutations.get(0).mutation().kind()).isEqualTo(tech.neural7.tracevanta.model.MutationKind.DELETE);
        assertThat(mutations.get(0).mutation().fidelity())
                .isEqualTo(tech.neural7.tracevanta.model.MutationFidelity.INFERRED);
    ...
    void publishesNothingWhenCaptureIsOff() throws Exception {
        TraceVantaConfig cfg = TraceVantaConfig.builder()
                ...
                .build();
        DataSource wrapped = TraceVantaJdbc.wrap(dataSource("DELETE FROM orders WHERE pk = 'x'", 1), cfg);
    ...
    void errorsAreRecordedAndPropagated() throws Exception {
        TraceVantaConfig cfg = TraceVantaConfig.defaults();
        DataSource wrapped = TraceVantaJdbc.wrap(dataSource("UPDATE orders SET x=1", null), cfg);
