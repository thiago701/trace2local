...
import tech.neural7.tracevanta.internal.ExecutionIds;
 ...
 * TraceVanta, para a execução aparecer na árvore em menos de um segundo.
 *
 * <p>O TraceVanta é, literalmente, um proxy que envia requisições por ordem do
...
public final class RequestLauncher implements ExecutionLauncher {
    ...
    public LaunchResult launch(ExecuteRequest request) throws Exception {
        ...
        var otel = tech.neural7.tracevanta.otel.TraceVantaOtel.get();
        var tracer = otel.getTracer("tech.neural7.tracevanta:launcher");
        Span span = tracer.spanBuilder("UI Dispatch: " + endpoint.method() + " " + endpoint.path())
                .setSpanKind(SpanKind.CLIENT)
                .setAttribute(TraceVantaAttributes.EXECUTION_ID, executionId)
        ...
        try (Scope ignored = span.makeCurrent()) {
            ...
            // 2) contexto W3C do TraceVanta por cima: traceparent/tracestate/baggage SEMPRE nossos,
            ...
            io.opentelemetry.api.baggage.Baggage baggage = io.opentelemetry.api.baggage.Baggage.builder()
                    .put("tracevanta.trigger", TraceVantaAttributes.TRIGGER_UI)
                    .build();
            ...
            span.setAttribute(tech.neural7.tracevanta.otel.OtelAttributeNames.HTTP_STATUS, response.statusCode());
