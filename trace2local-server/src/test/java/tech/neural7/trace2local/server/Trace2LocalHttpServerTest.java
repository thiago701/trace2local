...
class Trace2LocalHttpServerTest {
    ...
    void servesUiAndRestWithSecurityHeaders() throws Exception {
        ...
        try (Trace2LocalPipeline pipeline = Trace2LocalPipeline.start(cfg)) {
            Trace2LocalHttpServer server = Trace2LocalHttpServer.builder(cfg, pipeline)
                    .meta(() -> Trace2LocalMeta.embedded("order-service"))
            ...
            try {
                ...
                HttpResponse<String> index = httpGet(base + "/trace2local/");
                assertThat(index.statusCode()).isEqualTo(200);
                assertThat(index.body()).contains("TRACE2LOCAL");
                ...
                // meta
                HttpResponse<String> meta = httpGet(base + "/trace2local/api/meta");
                assertThat(meta.statusCode()).isEqualTo(200);
                assertThat(meta.body()).contains("order-service").contains("embedded");
                // descoberta programática da porta REAL (importante com trace2local.port=0)
                assertThat(meta.body()).contains("\"port\":" + server.port());

                // endpoints
                HttpResponse<String> endpoints = httpGet(base + "/trace2local/api/endpoints");
                assertThat(endpoints.body()).contains("post:/orders").contains("/orders");

                // health
                HttpResponse<String> health = httpGet(base + "/trace2local/api/health");
                assertThat(health.body()).contains("\"status\":\"ok\"").contains("\"connectedClients\":0");

                // execução inexistente ⇒ 404
                HttpResponse<String> missing = httpGet(base + "/trace2local/api/executions/TV-99999");
    ...
    void stationTokenProtectsExtraIngestRoutes() throws Exception {
        Trace2LocalConfig cfg = Trace2LocalConfig.builder().port(0).stationToken("sekrit-token").build();
        try (Trace2LocalPipeline pipeline = Trace2LocalPipeline.start(cfg)) {
            Trace2LocalHttpServer server = Trace2LocalHttpServer.builder(cfg, pipeline)
    ...
    void executeDelegatesToLauncherAndReturns202() throws Exception {
        ...
        try (Trace2LocalPipeline pipeline = Trace2LocalPipeline.start(cfg)) {
            ...
            server.start();
            try {
                HttpResponse<String> res = httpPost("http://127.0.0.1:" + server.port() + "/trace2local/api/execute",
    ...
    void streamCarriesLiveExecutionEvents() throws Exception {
        Trace2LocalConfig cfg = Trace2LocalConfig.builder().port(0).build();
        try (Trace2LocalPipeline pipeline = Trace2LocalPipeline.start(cfg)) {
            Trace2LocalHttpServer server = Trace2LocalHttpServer.builder(cfg, pipeline).build();
            server.start();
            try {
                ...
                var future = client.sendAsync(HttpRequest.newBuilder()
                                .uri(URI.create("http://127.0.0.1:" + server.port() + "/trace2local/api/stream"))
                                .GET().build(),
                ...
                pipeline.buffer().offer(new tech.neural7.trace2local.internal.SpanEndEvent(
